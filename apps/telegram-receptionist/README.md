...
restarts `telegram-intake.service` once. If locally modified tracked files
already match their versions in that exact revision, the worker safely stashes
those duplicate edits before fast-forwarding and drops the stash only after
HEAD reaches the target. Any other tracked production changes stop the
deployment and are reported for manual reconciliation.
